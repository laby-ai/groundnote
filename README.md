...
KnowTrail（灵笔）是一个面向资料研究、问答追溯和内容生成的 AI 工作本。它把 PDF、网页、音频、笔记和课堂素材收进同一个可引用的资料库，并在三栏工作台里完成资料对话、证据追溯、演示文稿、语音摘要、资料脉络和虚拟课堂生成。

![KnowTrail workspace](docs/screenshots/lingbi-workbench.png)

## 当前状态

- Live: [airai.world/lingbi](http://airai.world/lingbi)
- Runtime: `knowtrail.service` + `nginx`
- 最近线上巡检：页面、`/api/health`、工作台路由和虚拟课堂状态接口均返回 200。
- 当前定位：公网 C 端资料工作台。普通用户登录账号后使用工作本，不需要在浏览器里填写模型 API Key。

## 核心能力

- **资料库**：上传文档或粘贴资料，沉淀 source、chunk、页码和 ingestion 状态。
- **资料对话**：围绕选中资料提问，返回引用来源、检索状态和可追溯证据。
- **产物中心**：生成演示文稿、语音摘要、资料脉络和虚拟课堂，右栏保持任务配置和产物状态。
- **课堂工作流**：虚拟课堂草稿、运行态参数和工作台资料保持联动。
- **工程边界**：账号、模型、存储、向量检索和长任务状态有明确环境变量与 smoke 契约。
